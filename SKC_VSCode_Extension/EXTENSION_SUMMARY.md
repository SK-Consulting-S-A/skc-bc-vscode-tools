...
**SKC Workstation Tools** configures a single VS Code profile for SK Consulting employees and contractors: Business Central AL, Web/Python, and Power Platform/BI, with Copilot AI agents and skills, workspace presets, and XLF translation.
...
### 2. **Installs a Combined Extension Set**

The `extensions` array in `presets/settings.json` is the single source of truth for AL, Web/Python, and Power Platform/BI tooling. The extension has no manifest extension pack and does not install tools automatically on activation; everything installs together into the current (default) profile when **SKC: Apply Presets** runs.
...
### Automatic Setup
- **One-command setup** - Apply Presets installs the combined extension set, settings, and MCP servers together
...
- **Profile**: Installs the combined AL/Web/Python/Power Platform extension set into the current (default) VS Code profile
