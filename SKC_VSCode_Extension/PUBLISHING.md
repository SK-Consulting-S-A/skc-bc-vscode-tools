...
The current patch release is version **3.0.6**. Build and package it for review, then publish it with the configured Marketplace PAT. BCQuality is bundled and validated offline; live upstream refresh is opt-in through `skc.bcQualityUpdateOnApply` or the explicit update command.
